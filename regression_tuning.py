...
class RegressionTuner:
    """
    Class for hyperparameter tuning regression models using GridSearchCV.
    """
    ...
    def tune_models(self, X_train: pd.DataFrame, y_train: pd.Series, cv: int = 5) -> Dict[str, Any]:
        ...
        for name, info in self.models.items():
            ...
            if name in self.param_grids:
                grid = GridSearchCV(model, self.param_grids[name], cv=cv, scoring='neg_mean_squared_error')
